...
const (
	...
	loggerCfgDir           = "/etc/karpenter/config-logging"
...
func loggerFromFile(ctx context.Context, component string) *zap.SugaredLogger {
	...
	raw, err = os.ReadFile(loggerCfgFilePath + fmt.Sprintf("loglevel.%s", component))
	if err != nil && !os.IsNotExist(err) {
		log.Fatalf("retrieving logging controller log level file from %q", loggerCfgFilePath+fmt.Sprintf("loglevel.%s", component))
	}
	if raw != nil {
