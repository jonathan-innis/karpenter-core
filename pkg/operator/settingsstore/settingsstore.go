...
func (s *store) InjectSettings(ctx context.Context) context.Context {
	return lo.Reduce(s.registrations, func(c context.Context, registration *config.Registration, _ int) context.Context {
		return context.WithValue(c, registration, lo.Ternary(
			s.stores[registration].UntypedLoad(registration.ConfigMapName) != nil,
			s.stores[registration].UntypedLoad(registration.ConfigMapName),
