...
var _ = Describe("Eviction/Queue", func() {
	...
	Context("Eviction API", func() {
		It("should succeed with no event when the pod is not found", func() {
			...
			Expect(queue.Evict(ctx, terminator.QueueKey{NamespacedName: types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}})).To(BeTrue())
		...
		It("should return a NodeDrainError event when a PDB is blocking", func() {
			...
			Expect(queue.Evict(ctx, terminator.QueueKey{NamespacedName: types.NamespacedName{Name: pod.Name, Namespace: pod.Namespace}})).To(BeFalse())
