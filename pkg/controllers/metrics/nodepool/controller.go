...
const (
	nodePoolResourceType = "resource_type"
...
var (
	...
	usagePctGaugeVec = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			...
			Subsystem: "nodepool",
			Name:      "usage_pct",
			Help:      "The NodePool Usage Percentage is the percentage of each resource used based on the resources provisioned and the limits that have been configured in the range [0,100]. Labeled by NodePool name and resource type.",
		},
		labelNames(),
	)
)
...
func (c *Controller) record(ctx context.Context, nodePool *v1beta1.NodePool) {
	if err := c.set(nodePool, nodePool.Status.Resources, usageGaugeVec); err != nil {
	...
	if err := c.set(nodePool, v1.ResourceList(nodePool.Spec.Limits), limitGaugeVec); err != nil {
		logging.FromContext(ctx).Errorf("generating gauge: %s", err)
	}
	usage := v1.ResourceList{}
	for k, v := range nodePool.Spec.Limits {
	...
	if err := c.set(nodePool, usage, usagePctGaugeVec); err != nil {
		logging.FromContext(ctx).Errorf("generating gauge: %s", err)
	}
}

// set updates the value for the node gauge for each resource type passed through the resource list
func (c *Controller) set(nodePool *v1beta1.NodePool, resourceList v1.ResourceList, gaugeVec *prometheus.GaugeVec) error {
	provisionerKey := client.ObjectKeyFromObject(nodePool)

	var labels []prometheus.Labels
	for resourceName, quantity := range resourceList {
		resourceTypeName := strings.ReplaceAll(strings.ToLower(string(resourceName)), "-", "_")

		label := c.makeLabels(nodePool, resourceTypeName)
...
func (c *Controller) makeLabels(nodePool *v1beta1.NodePool, resourceTypeName string) prometheus.Labels {
	return prometheus.Labels{
		nodePoolResourceType: resourceTypeName,
