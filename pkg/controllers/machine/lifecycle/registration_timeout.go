...
func (r *RegistrationTimeout) Reconcile(ctx context.Context, machine *v1alpha5.Machine) (reconcile.Result, error) {
	...
	if r.clock.Since(machine.StatusConditions().GetCondition(v1alpha5.MachineLaunched).LastTransitionTime.Inner.Time) < registrationTTL {
		return reconcile.Result{RequeueAfter: registrationTTL - r.clock.Since(machine.StatusConditions().GetCondition(v1alpha5.MachineLaunched).LastTransitionTime.Inner.Time)}, nil
	}
