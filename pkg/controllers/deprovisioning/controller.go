...
func NewController(clk clock.Clock, kubeClient client.Client, provisioner *provisioning.Provisioner,
	...
	return &Controller{
		...
		deprovisioners: []Deprovisioner{
			...
			// Delete any remaining empty nodes as there is zero cost in terms of disruption.  Emptiness and
...
func (c *Controller) executeCommand(ctx context.Context, d Deprovisioner, command Command) error {
	...
	if command.action == actionReplace {
		if err := c.launchReplacementMachines(ctx, command); err != nil {
	...
	for _, oldNode := range command.nodesToRemove {
		...
		if err := c.kubeClient.Delete(ctx, oldNode.Machine); err != nil {
			logging.FromContext(ctx).Errorf("Deleting machine, %s", err)
		} else {
			metrics.MachinesTerminatedCounter.WithLabelValues(fmt.Sprintf("%s/%s", d, command.action)).Inc()
		}
	}
...
// nolint:gocyclo
func (c *Controller) launchReplacementMachines(ctx context.Context, action Command) error {
	...
	machineNames, err := c.provisioner.LaunchMachines(ctx, action.replacementMachines)
	if err != nil {
	...
	metrics.MachinesCreatedCounter.WithLabelValues(metrics.DeprovisioningReason).Add(float64(len(machineNames)))
