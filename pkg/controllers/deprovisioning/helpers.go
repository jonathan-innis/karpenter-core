...
func filterCandidates(ctx context.Context, kubeClient client.Client, recorder events.Recorder, nodes []*Candidate) ([]*Candidate, error) {
	...
	nodes = lo.Filter(nodes, func(cn *Candidate, _ int) bool {
		if !cn.Node.DeletionTimestamp.IsZero() {
			recorder.Publish(deprovisioningevents.Blocked(ctx, cn.Node, cn.Machine, "in the process of deletion")...)
			return false
		}
		if pdb, ok := pdbs.CanEvictPods(cn.pods); !ok {
			recorder.Publish(deprovisioningevents.Blocked(ctx, cn.Node, cn.Machine, fmt.Sprintf("pdb %s prevents pod evictions", pdb))...)
			return false
		}
		if p, ok := hasDoNotEvictPod(cn); ok {
			recorder.Publish(deprovisioningevents.Blocked(ctx, cn.Node, cn.Machine, fmt.Sprintf("pod %s/%s has do not evict annotation", p.Namespace, p.Name))...)
			return false
		}
		return true
	})
