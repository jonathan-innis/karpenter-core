...
func NewCandidate(ctx context.Context, kubeClient client.Client, recorder events.Recorder, clk clock.Clock, node *state.StateNode,
	...
	if node.MarkedForDeletion() {
		recorder.Publish(deprovisioningevents.Blocked(node.Node, node.NodeClaim, "machine is marked for deletion")...)
	...
	if !node.Initialized() {
		recorder.Publish(deprovisioningevents.Blocked(node.Node, node.NodeClaim, "machine is not initialized")...)
	...
	if node.Nominated() {
		recorder.Publish(deprovisioningevents.Blocked(node.Node, node.NodeClaim, "machine is nominated")...)
