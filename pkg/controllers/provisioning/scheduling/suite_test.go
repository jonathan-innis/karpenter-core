...
var _ = Describe("Custom Constraints", func() {
	...
	Context("Constraints Validation", func() {
		...
		It("should schedule pods that have node selectors with label in wellknown label list", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			for _, pod := range ExpectProvisioned(ctx, env.Client, recorder, controller, prov, schedulable...) {
				ExpectScheduled(ctx, env.Client, pod)
			}
		})
	})
...
var _ = Describe("Topology", func() {
	...
	Context("Zonal", func() {
		...
		It("should only schedule to minimum domains if already violating max skew", func() {
			...
			// Spread 9 pods
			ExpectApplied(ctx, env.Client, provisioner)
			pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, createPods(9)...)
			...
			_ = ExpectProvisioned(ctx, env.Client, recorder, controller, prov, createPods(3)...)
	...
	Context("Hostname", func() {
		...
		It("balance multiple deployments with hostname topology spread", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			scheduled := ExpectProvisioned(ctx, env.Client, recorder, controller, prov,
	...
	Context("CapacityType", func() {
		...
		It("should balance pods across capacity-types (node required affinity constrained)", func() {
			ExpectApplied(ctx, env.Client, provisioner)
			pod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, MakePods(1, test.PodOptions{
		...
		It("should balance pods across capacity-types (no constraints)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			pod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(test.PodOptions{
				ObjectMeta: metav1.ObjectMeta{Labels: labels},
				NodeRequirements: []v1.NodeSelectorRequirement{
					{
						Key:      v1alpha5.LabelCapacityType,
	...
	Context("Combined Hostname and Zonal Topology", func() {
		...
		It("should balance pods across provisioner requirements", func() {
			...
			pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, MakePods(20, test.PodOptions{
				ObjectMeta:                metav1.ObjectMeta{Labels: labels},
				TopologySpreadConstraints: topology,
			})...)
			for _, p := range pods {
	...
	Context("Combined Hostname, Zonal, and Capacity Type Topology", func() {
		It("should spread pods while respecting all constraints", func() {
			...
			for i := 1; i < 15; i++ {
				...
				ExpectApplied(ctx, env.Client, provisioner)
				ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pods...)
	...
	Context("Combined Zonal Topology and Node Affinity", func() {
		...
		It("should limit spread options by node affinity", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, MakePods(1, test.PodOptions{
				ObjectMeta:                metav1.ObjectMeta{Labels: labels},
				TopologySpreadConstraints: topology,
				NodeRequirements: []v1.NodeSelectorRequirement{{Key: v1.LabelTopologyZone, Operator: v1.NodeSelectorOpIn, Values: []string{
	...
	Context("Pod Affinity/Anti-Affinity", func() {
		...
		It("should respect self pod affinity (hostname)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			pods = ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pods...)
		...
		It("should respect self pod affinity for first empty topology domain only (hostname)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, createPods()...)
			nodeNames := map[string]struct{}{}
			unscheduledCount := 0
			scheduledCount := 0
			for _, p := range pods {
		...
		It("should respect self pod affinity for first empty topology domain only (hostname/constrained zones)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(test.PodOptions{
				ObjectMeta: metav1.ObjectMeta{
					Labels: affLabels,
				},
				NodeSelector: map[string]string{
		...
		It("should separate nodes using simple pod anti-affinity on hostname", func() {
			...
			for i := 0; i < 10; i++ {
				...
				ExpectProvisioned(ctx, env.Client, recorder, controller, prov, affPod2, affPod1)
				n1 := ExpectScheduled(ctx, env.Client, affPod1)
				n2 := ExpectScheduled(ctx, env.Client, affPod2)
		...
		It("should not violate pod anti-affinity on zone", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, zone1Pod, zone2Pod, zone3Pod, affPod)
		...
		It("should not violate pod anti-affinity on zone (other schedules first)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pod, affPod)
		...
		It("should not violate pod anti-affinity on zone (Schrödinger)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, zoneAnywherePod, affPod)
			...
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, affPod)
		...
		It("should not violate pod anti-affinity on zone (inverse w/existing nodes)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, zone1Pod, zone2Pod, zone3Pod)
		...
		It("should allow violation of a pod affinity preference with a conflicting required constraint", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, append(affPods, affPod1)...)
		...
		It("should support pod anti-affinity with a zone topology", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, createPods()...)
		...
		It("should not schedule pods with affinity to a non-existent pod", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, affPods...)
		...
		It("should support pod affinity with zone topology (unconstrained target)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, append(affPods, targetPod)...)
			top := &v1.TopologySpreadConstraint{TopologyKey: v1.LabelTopologyZone}
			...
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, affPods...)
		...
		It("should count topology across multiple provisioners", func() {
			...
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.Pods(10, test.UnscheduleablePodOptions(test.PodOptions{
				ObjectMeta:                metav1.ObjectMeta{Labels: labels},
...
var _ = Describe("Networking constraints", func() {
	Context("HostPort", func() {
		It("shouldn't co-locate pods that use the same HostPort and protocol (default protocol)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pod1, pod2)
		...
		It("shouldn't co-locate pods that use the same HostPort but a different IP, where one ip is 0.0.0.0 (existingNodes)", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pod1)
			node1 := ExpectScheduled(ctx, env.Client, pod1)
			ExpectReconcileSucceeded(ctx, nodeStateController, client.ObjectKeyFromObject(node1))

			ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pod2)
			node2 := ExpectScheduled(ctx, env.Client, pod2)
			Expect(node1.Name).ToNot(Equal(node2.Name))
		})
...
var _ = Describe("Binpacking", func() {
	...
	It("should schedule multiple small pods on the smallest possible instance type", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.Pods(5, opts)...)
		nodeNames := sets.NewString()
		for _, p := range pods {
	...
	It("should create new nodes when a node is at capacity", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.Pods(40, opts)...)
		nodeNames := sets.NewString()
		for _, p := range pods {
			node := ExpectScheduled(ctx, env.Client, p)
			nodeNames.Insert(node.Name)
	...
	It("should pack small and large pods together", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, provPods...)
		nodeNames := sets.NewString()
		for _, p := range pods {
			node := ExpectScheduled(ctx, env.Client, p)
			nodeNames.Insert(node.Name)
	...
	It("should create new nodes when a node is at capacity due to pod limits per node", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		pods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.Pods(25, opts)...)
		nodeNames := sets.NewString()
...
var _ = Describe("In-Flight Nodes", func() {
	It("should not launch a second node if there is an in-flight node that can support the pod", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		initialPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(opts))
		node1 := ExpectScheduled(ctx, env.Client, initialPod[0])
		ExpectReconcileSucceeded(ctx, nodeStateController, client.ObjectKeyFromObject(node1))

		secondPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(opts))
	...
	It("should not launch a second node if there is an in-flight node that can support the pod (node selectors)", func() {
		ExpectApplied(ctx, env.Client, provisioner)
		initialPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(test.PodOptions{ResourceRequirements: v1.ResourceRequirements{
			Limits: map[v1.ResourceName]resource.Quantity{
				v1.ResourceCPU: resource.MustParse("10m"),
			},
		},
		...
		secondPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(test.PodOptions{ResourceRequirements: v1.ResourceRequirements{
			Limits: map[v1.ResourceName]resource.Quantity{
				v1.ResourceCPU: resource.MustParse("10m"),
			},
		},
		...
		// the node gets created in test-zone-2
		thirdPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(test.PodOptions{ResourceRequirements: v1.ResourceRequirements{
			Limits: map[v1.ResourceName]resource.Quantity{
				v1.ResourceCPU: resource.MustParse("10m"),
			},
		},
	...
	It("should launch a second node if a pod isn't compatible with the existingNodes node (node selector)", func() {
		...
		secondPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov,
			test.UnschedulablePod(test.PodOptions{NodeSelector: map[string]string{v1.LabelArchStable: "arm64"}}))
		node2 := ExpectScheduled(ctx, env.Client, secondPod[0])
		Expect(node1.Name).ToNot(Equal(node2.Name))
	})
	...
	Context("Taints", func() {
		It("should assume pod will schedule to a tainted node with no taints", func() {
			...
			secondPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod())
			node2 := ExpectScheduled(ctx, env.Client, secondPod[0])
			Expect(node1.Name).To(Equal(node2.Name))
		})
		...
		It("should not assume pod will schedule to a node with startup taints after initialization", func() {
			...
			ExpectApplied(ctx, env.Client, provisioner)
			initialPod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod())
			node1 := ExpectScheduled(ctx, env.Client, initialPod[0])
		...
		It("should consider a tainted NotReady node as in-flight even if initialized", func() {
			...
			pod := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod(opts))[0]
	...
	It("should pack in-flight nodes before launching new nodes", func() {
		...
		for i := 0; i < 10; i++ {
			initialPods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, MakePods(rand.Intn(10), opts)...)
	...
	It("should not launch a second node if there is an in-flight node that can support the pod (#2011)", func() {
		...
		ExpectProvisionedNoBinding(ctx, env.Client, controller, prov, pod)
		var nodes v1.NodeList
		Expect(env.Client.List(ctx, &nodes)).To(Succeed())
...
var _ = Describe("No Pre-Binding", func() {
	It("should not bind pods to nodes", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		initialPod := ExpectProvisionedNoBinding(ctx, env.Client, controller, prov, test.UnschedulablePod(opts))
		...
		ExpectReconcileSucceeded(ctx, nodeStateController, client.ObjectKeyFromObject(node1))
		secondPod := ExpectProvisionedNoBinding(ctx, env.Client, controller, prov, test.UnschedulablePod(opts))
	...
	It("should respect self pod affinity without pod binding (zone)", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		ExpectProvisionedNoBinding(ctx, env.Client, controller, prov, pods[0])
		var nodeList v1.NodeList
		Expect(env.Client.List(ctx, &nodeList)).To(Succeed())
		...
		ExpectProvisionedNoBinding(ctx, env.Client, controller, prov, pods[1])
		Expect(env.Client.List(ctx, &nodeList)).To(Succeed())
		Expect(nodeList.Items).To(HaveLen(1))
	})
})
...
var _ = Describe("Volume Limits", func() {
	It("should launch multiple nodes if required due to volume limits", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		initialPods := ExpectProvisioned(ctx, env.Client, recorder, controller, prov, test.UnschedulablePod())
	...
	It("should not fail for non-dynamic PVCs", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner)
		_ = ExpectProvisioned(ctx, env.Client, recorder, controller, prov, pods...)
