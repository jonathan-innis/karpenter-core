...
func NewFailedInit(clk clock.Clock, cluster *state.Cluster) Check {
	return &FailedInit{clock: clk, cluster: cluster}
}
...
func (f FailedInit) Check(ctx context.Context, n *v1.Node, provisioner *v1alpha5.Provisioner, pdbs *deprovisioning.PDBLimits) ([]Issue, error) {
	...
	stateNode := f.cluster.Get(n.Spec.ProviderID)
	if stateNode == nil || stateNode.Node == nil || stateNode.Machine == nil {
		return nil, nil
	}

	// detect startup taints which should be removed
	var result []Issue
	if taint, ok := node.IsStartupTaintRemoved(stateNode.Node, stateNode.Machine); !ok {
	...
	if resource, ok := node.RequestedResourcesRegistered(stateNode.Node, stateNode.Machine); !ok {
		result = append(result, Issue{
			node:    n,
