...
//			provisioner.Spec.GetLabels = map[string]string{"foo": randomdata.SillyName()}
//			Expect(provisioner.Validate(ctx)).To(Succeed())
//		})
//		It("should fail for the provisioner name label", func() {
//			provisioner.Spec.GetLabels = map[string]string{ProvisionerNameLabelKey: randomdata.SillyName()}
//			Expect(provisioner.Validate(ctx)).ToNot(Succeed())
//		})
//		It("should fail for invalid label keys", func() {
//			provisioner.Spec.GetLabels = map[string]string{"spaces are not allowed": randomdata.SillyName()}
//			Expect(provisioner.Validate(ctx)).ToNot(Succeed())
//		})
//		It("should fail for invalid label values", func() {
//			provisioner.Spec.GetLabels = map[string]string{randomdata.SillyName(): "/ is not allowed"}
//			Expect(provisioner.Validate(ctx)).ToNot(Succeed())
//		})
//		It("should fail for restricted label domains", func() {
//			for label := range RestrictedLabelDomains {
//				provisioner.Spec.GetLabels = map[string]string{label + "/unknown": randomdata.SillyName()}
//				Expect(provisioner.Validate(ctx)).ToNot(Succeed())
//			}
//		})
//		It("should allow labels kOps require", func() {
//			provisioner.Spec.GetLabels = map[string]string{
